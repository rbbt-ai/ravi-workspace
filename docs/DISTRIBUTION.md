...
python3 scripts/distribution.py build --out ../releases/ravi-workspace-0.1.0-alpha.8.tar.gz
```
...
```sh
python3 scripts/distribution.py verify ../releases/ravi-workspace-0.1.0-alpha.8.tar.gz --sha256 <sha256-do-recibo>
```
...
O diretório de App é `src/apps/workspace-hub/`. A CLI nativa descobre esse diretório em `apps/<id>` do estado Ravi, mas não foi encontrado instalador/atualizador oficial. Não substituir diretórios existentes nem criar grants automaticamente. Instalação, atualização, backup e recuperação do pacote estão descritos em [INSTALLATION.md](INSTALLATION.md).
