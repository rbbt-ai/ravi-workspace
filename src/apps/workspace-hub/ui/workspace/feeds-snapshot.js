  ...
  let feedLiveStates=Object.fromEntries(Object.entries(data.sourceStates).map(([key,s])=>[key,{verified:s.status==='ready',error:s.status==='unavailable'?'SOURCE_UNAVAILABLE':null,stale:s.status==='stale',data:{...data.home.feeds[key],capturedAt:s.capturedAt}}]));
